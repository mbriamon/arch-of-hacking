...
\pgfmathsetmacro{\titley}{\cardheight-\strippadding-5*\textpadding-0.4*\stripwidth}
...
\newcommand{\cardtitle}[1]{
    % Transparent title background, old approach
    %\draw[pattern=soft crosshatch,rounded corners=0.1cm] \shapeTitleArea;

    % Transparent title background, new approach
    % Commented in favor of the tcolorbox approach
    \fill[elementcorners,titlebg,opacity=.6] \shapeTitleArea;

    % Title text area
    %\node[text width=3.75cm] at (0.5*\cardwidth,\titley) {
    \node[text width=3.90cm] at (0.5*\cardwidth,\titley) {
        \begin{center}
            % Normal
